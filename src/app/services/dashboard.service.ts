...
export class DashboardService {
  ...
  getAllNominations() {
    return this.http.get(this.GET_ALL_NOMINATIONS_LIST);
    // body 里面是object的 list
  }

  passNominee(nominee: Nominee) {
    return this.http.put(this.PUT_NOMINATION_PASSED + nominee.id, nominee);
  }

  updateNominee(nominee: Nominee) {
    return this.http.put(this.PUT_NOMINATION, nominee);
  }

  deleteNominee(nominee: Nominee) {
    return this.http.delete(this.DELETE_NOMINATION_BY_ID + nominee.id);
  }


  addSchool(school: School) {
    return this.http.post(this.POST_SCHOOL, school);
  }
  deleteSchool(school: School) {
    return this.http.delete(this.DELETE_SCHOOL_BY_ID + school.id);
  }

  addType(t: Type) {
    return this.http.post(this.POST_NOMINEE_TYPE, t);
  }
  deleteType(t: Type) {
    return this.http.delete(this.DELETE_NOMINATION_BY_ID + t.id);
  }
